...
public class LearningChapter2 extends Learning {
	...
	public void quiz(int question_num) {
		...
		question_pane.add(ch1.getChap2question().get(question_num));
		JLabel q_imageLabel = new JLabel();
		q_imageLabel.setIcon(ch1.getChap2Image().get(question_num));
		ch1.getChap2question().get(question_num).setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		for (int i = 0; i < ch1.getChap2choice()[question_num].length; i++)
			random_num_array.add(i);
		Collections.shuffle(random_num_array);
		for (choice_num = 0; choice_num < ch1.getChap2choice()[question_num].length; choice_num++) {
			choicePane.add(ch1.getChap2choice()[question_num][(random_num_array.get(choice_num))]);
			ch1.getChap2choice()[question_num][(random_num_array.get(choice_num))].addActionListener(this);
		}
	...
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		if (e.getSource().equals(ch1.getChap2correct().get(question_num))) {
			answer = ch1.getChap2correct().get(question_num).getText(); // save your answer(correct answer)
			for (int i = 0; i < ch1.getChap2choice()[question_num].length; i++) {
				ch1.getChap2choice()[question_num][i].setForeground(Color.WHITE);
				ch1.getChap2choice()[question_num][i].setBackground(new Color(127, 255, 0));
			}
			ch1.getChap2correct().get(question_num).setForeground(Color.RED);
			ch1.getChap2correct().get(question_num).setBackground(new Color(126, 187, 0));
		} else {
			for (int r = 1; r < ch1.getChap2choice()[question_num].length; r++)
				if (e.getSource().equals(ch1.getChap2choice()[question_num][r])) {
					answer = ch1.getChap2choice()[question_num][r].getText(); // save your answer(incorrect answer)
					for (int w = 0; w < ch1.getChap2choice()[question_num].length; w++) {
						ch1.getChap2choice()[question_num][w].setForeground(Color.WHITE);
						ch1.getChap2choice()[question_num][w].setBackground(new Color(127, 255, 0));
					}
					ch1.getChap2choice()[question_num][r].setForeground(Color.RED);
					ch1.getChap2choice()[question_num][r].setBackground(new Color(126, 187, 0));
				}
		}
		...
		if (e.getSource().equals(submit)) {
			...
			if (answer == ch1.getChap2correct().get(question_num).getText()) {
		...
		if (e.getSource().equals(jButton_back)) {
			...
			for (int i = 0; i < ch1.getChap2choice().length; i++) {
				for (int j = 0; j < ch1.getChap2choice()[i].length; j++) {
					ch1.getChap2choice()[i][j].setForeground(Color.WHITE);
					ch1.getChap2choice()[i][j].setBackground(new Color(127, 255, 0));
				}
			}
		}
		...
		if (e.getSource().equals(next)) {
			...
			// screen.
			if (question_num >= ch1.getChap2question().size() - 1) {
				JOptionPane.showMessageDialog(null, "Chapter 2 is END!","message",1);
	...
	public void showAnswers(JPanel pane) {
		...
		JLabel correct_answer = new JLabel(
				"<html>The correct answer is : <br>" + ch1.getChap2correct().get(question_num).getText() + "</html>");
	...
	public void showExplanation(JPanel pane) {
		...
		remember_pane.add(ch1.getChap2remember().get(question_num));
		...
		solve_pane.add(ch1.getChap2solve().get(question_num));
